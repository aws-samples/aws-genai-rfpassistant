...
export interface ChatInputPanelProps {
  ...
  setDownloadResponsesStatus: Dispatch<SetStateAction<boolean>>
}
      ...
      >({
        ...
        next: ({value} : {value:any}) => {
          ...
          if (data !== undefined && data !== null) {
            ...
            if ( response.action === ChatBotAction.FinalResponse || response.action === ChatBotAction.Error ) {
              console.log("Final message received");
              props.setRunning(false);
              props.setDownloadResponsesStatus(true)
            }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        */ 
        const selectedWorkspaceOption = OptionsHelper.getSelectOptions([
          workspaces[0],
        ])[0]
...
*/
  const handleSendMessage = () => {
    if(state.value.trim().length === 0 || files.length === 0) return;
        ...
        <Container>
          <div className={styles.input_textarea_container}>
            ...
            <TextareaAutosize
              ...
              value={state.value}
              placeholder={listening ? "Listening..." : "Add a file and Provide a title or reference id"}
            />
            <div style={{ marginLeft: "8px" }}>
              ...
              <Button
                disabled={
                  ...
                  !state.selectedModel ||
                  props.running ||
                  (state.value.trim().length === 0 || files.length === 0)||
  ...
  return((props.messageHistory.length == 0)? renderInputPanel() : null  )
}
/*
