...
export default function ImageDialog(props: ImageDialogProps) {
  ...
  const uploadFile = async (file: File) => {
    ...
    const fileKey = `${shortId}.${extension}`;
    const response = await Storage.put(fileKey, file, {
      contentType,
    });

    const s3URI = `s3://${Amplify.Storage["_config"]["AWSS3"]["bucket"]}/public/${fileKey}`;
      ...
      <Form>
        <SpaceBetween size="m">
          <FormField
            ...
            description="You can upload an excel file to be used for this conversation."
          >
            <FileUpload
              onChange={({ detail }) => {
              ...
              tokenLimit={3}
              constraintText=".xlsx, .xls,  Max 10MB."
            />
          </FormField>
          {loading && (
            <>
