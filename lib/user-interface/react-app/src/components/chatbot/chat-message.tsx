  ...
  return (
    <div>
      ...
      {/* {files && !loading && (
        <>
          {files.map((file, idx) => (
            <div key={`div_rfp_${idx}`} className={styles.div_chatbot_title}>
              <a key={idx} href={file.url as string} target="_blank" rel="noreferrer">
                <img src="/images/file_icon.svg" className={styles.img_chatbot_file_message} />
                <div className={styles.div_chatbot_filename}>{file.key}</div>
              </a>
            </div>
