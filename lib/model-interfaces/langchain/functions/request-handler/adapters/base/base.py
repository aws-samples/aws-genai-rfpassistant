...
class ModelAdapter:
    def __init__(
        self, session_id, user_id, session_type, mode=ChatbotMode.CHAIN.value, model_kwargs={}
    ):
    ...
    def run_with_chain(self, user_prompt, workspace_id=None,companyName=None):
        if not self.llm:
        ...
        if workspace_id:
            ...
            if self.session_type == "chat": 
                conversation = ConversationalRetrievalChain.from_llm(
                    ...
                    combine_docs_chain_kwargs={"prompt": self.get_qa_prompt({"CompanyName":companyName})},
                ...
                #use Retrival QA for RFP sessiontype            
                qa = RetrievalQA.from_chain_type(
                        llm=self.llm,
                        chain_type="stuff",
                        retriever=WorkspaceRetriever(workspace_id=workspace_id),
                        return_source_documents=True,
                        chain_type_kwargs={"prompt": self.get_qa_prompt({"CompanyName":companyName})},
                        callbacks=[self.callback_handler]
                    )

                print(f"received query : {user_prompt}")
    ...
    def run(self, prompt, workspace_id=None, companyName=None,*args, **kwargs):
        ...
        if self._mode == ChatbotMode.CHAIN.value:
            return self.run_with_chain(prompt, workspace_id,companyName=companyName)
